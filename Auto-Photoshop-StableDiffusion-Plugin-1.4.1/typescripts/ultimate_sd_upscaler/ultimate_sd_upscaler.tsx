    ...
    render() {
            ...
            return (
                <div>
                    ...
                    <button
                        ...
                        title={Locale('Refresh the Ultimte SD Upscale script')}
        ...
        const group_1_sliders = ids.map((id) => (
            <SpSliderWithLabel
                ...
                title={Locale(ui_config[id].label)}
                label={Locale(ui_config[id].label)}
                onSliderInput={this.handleSliderChange}
            />
        ))
        const seamfix_ids = [
        ...
        return (
            <div>
                <SpMenu
                    title={Locale('Stable Diffusion Upscalers')}
                    items={this.state.sd_upscalers}
                    label_item={Locale('Select Upscaler')}
                    id={'upscaler_index'}
                    onChange={this.handleMenuChange}
                    selected_index={this.props.store.data.upscaler_index}
                />
                <SpMenu
                    title={Locale(ui_config.target_size_type.label)}
                    id={'target_size_type'}
                    items={ui_config.target_size_type.choices}
                    label_item={
                        Locale('Select') +
                        ' ' +
                        Locale(ui_config.target_size_type.label)
                    }
                    onChange={this.handleMenuChange}
                ...
                <div>
                    ...
                    >
                        {Locale(ui_config.save_upscaled_image.label)}
                    </sp-checkbox>
                    <sp-checkbox
                        checked={
                    ...
                    >
                        {Locale(ui_config.save_seams_fix_image.label)}
                    </sp-checkbox>
                </div>
                {group_1_sliders}
                <SpMenu
                    title={Locale('Seams Fix Type')}
                    id={'seams_fix_type'}
                    items={ui_config.seams_fix_type.choices}
                    label_item={Locale('Select Seams Fix Type')}
                    onChange={this.handleMenuChange}
