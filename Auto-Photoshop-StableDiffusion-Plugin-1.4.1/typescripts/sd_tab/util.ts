...
export function loadPresetSettings(preset: any) {
    if (preset?.sd_tab_preset) {
        const {
            ...
            current_index,
            ...sd_tab_settings
        } = preset.sd_tab_preset

        presetToStore(sd_tab_settings, store)

        if (prompts?.positivePrompts && prompts?.negativePrompts) {
            multi_prompt_store.data.positivePrompts = prompts.positivePrompts
            multi_prompt_store.data.negativePrompts = prompts.negativePrompts
            multi_prompt_store.data.current_index = prompts.current_index ?? 0
        } else if (positivePrompts && negativePrompts) {
            multi_prompt_store.data.positivePrompts = positivePrompts
            multi_prompt_store.data.negativePrompts = negativePrompts
            multi_prompt_store.data.current_index = current_index ?? 0
        } else {
            setPrompt({
