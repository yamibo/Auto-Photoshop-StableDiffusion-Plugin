...
import {
    ...
    moveCustomPresetOrder,
    renameCustomPresetSettings,
    saveCustomPresetSettings,
} from '../preset/shared_ui_preset'
import {
    createGenerationPreset,
    initLastGenerationSettingsAutosave,
    loadLastGenerationSettings,
} from '../preset/generation_settings'
...
function getTimestampPresetName() {
    return `preset_${new Date().toISOString().replace(/[:.]/g, '-')}`
}

async function saveCurrentCustomPreset() {
    const preset_name =
        preset_store.data.preset_name?.trim() || getTimestampPresetName()

    preset_store.data.custom_presets = await saveCustomPresetSettings(
        preset_name,
        createGenerationPreset()
    )
    preset_store.data.preset_name = preset_name
    preset_store.data.selected_sd_preset_name = preset_name
    preset_store.data.selected_sd_preset =
        preset_store.data.custom_presets[preset_name]
}

function getCustomPresetNames() {
    return Object.keys(preset_store.data.custom_presets || {})
}

function isSelectedCustomPreset() {
    return getCustomPresetNames().includes(
        preset_store.data.selected_sd_preset_name
    )
}

async function deleteSelectedCustomPreset() {
    const selected_preset_name = preset_store.data.selected_sd_preset_name
    if (!isSelectedCustomPreset()) return

    preset_store.data.custom_presets = await deleteCustomPresetSettings(
        selected_preset_name
    )
    preset_store.data.selected_sd_preset_name = ''
    preset_store.data.selected_sd_preset = {}
    preset_store.data.preset_name = ''
}

async function renameSelectedCustomPreset() {
    const selected_preset_name = preset_store.data.selected_sd_preset_name
    const new_preset_name = preset_store.data.preset_name?.trim()
    if (!isSelectedCustomPreset() || !new_preset_name) return

    preset_store.data.custom_presets = await renameCustomPresetSettings(
        selected_preset_name,
        new_preset_name
    )
    preset_store.data.selected_sd_preset_name = new_preset_name
    preset_store.data.selected_sd_preset =
        preset_store.data.custom_presets[new_preset_name]
}

async function moveSelectedCustomPreset(direction: -1 | 1) {
    const selected_preset_name = preset_store.data.selected_sd_preset_name
    if (!isSelectedCustomPreset()) return

    preset_store.data.custom_presets = await moveCustomPresetOrder(
        selected_preset_name,
        direction
    )
}
...
class SDTab extends React.Component<{}> {
    ...
    render() {
        ...
        const native_preset_names = Object.keys(
            helper_store.data.native_presets || {}
        )
        const custom_preset_names = getCustomPresetNames()
        const preset_names = [...native_preset_names, ...custom_preset_names]
        const selected_custom_preset_index = custom_preset_names.indexOf(
            preset_store.data.selected_sd_preset_name
        )
        const selected_is_custom = selected_custom_preset_index >= 0
        return (
            <div>
                <div
                ...
                >
                    ...
                    ></SpMenu> */}
                        <SearchableMenu
                            ...
                            placeholder={Locale('Select a Model')}
                    ...
                    <div style={{ display: 'flex', alignItems: 'flex-start' }}>
                        <button
                            style={{ padding: '3px' }}
                            title={Locale(
                                'Refresh the plugin, only fixes minor issues.'
                            )}
                            id="btnRefreshModels"
                            // style={styles.button}
                            onClick={async (e) => {
                        ...
                        <button
                            style={{ padding: '3px' }}
                            title={Locale(
                                'Update the plugin if you encounter bugs. Get the latest features'
                            )}
                            className="btnSquare"
                            id="btnUpdate"
                            // style={styles.button}
                ...
                <div id="sdBtnContainer">
                    ...
                    <SearchableMenu
                        ...
                        placeholder={Locale('Select Lora')}
                        onChange={(item: any) => {
                    ...
                    <sp-checkbox
                        title={Locale('use {keyword} form the prompts library')}
                ...
                <div>
                    <div id="progressContainer">
                        <div style={{ display: 'flex' }}>
                            ...
                            <input
                                title={Locale(
                                    'Toggle the visibility of the Preview Image on the canvas'
                                )}
                                type="checkbox"
                                name="optionCheckbox"
                                checked={
                ...
                <div>
                        ...
                        >
                            ...
                            >
                                ...
                                <SpTextfield
                                    style={{ width: '100%' }}
                                    title={Locale(
                                        'the number of images to generate in queue. The larger the number the longer will take.'
                                    )}
                            ...
                            >
                                ...
                                <SpTextfield
                                    style={{ width: '100%' }}
                                    title={Locale(
                                        'the higher the steps the longer it will take to generate an image'
                                    )}
                                    id="tiNumberOfSteps"
                                    type="number"
                                    placeholder="20"
                    ...
                    <div id="selectionMode">
                        ...
                        >
                            ...
                            <div id="sdPresetContainer">
                                <SpMenu
                                    ...
                                    selected_index={preset_names.indexOf(
                                        preset_store.data
                                ...
                                >
                                    <SpTextfield
                                        ...
                                        value={preset_store.data.preset_name}
                                        onInput={(evt: any) => {
                                            preset_store.data.preset_name =
                                                evt.target.value
                                        }}
                                    ...
                                    <button
                                        className="btnSquare"
                                        title={Locale(
                                            'Save current prompts and generation settings as a custom preset'
                                        )}
                                        onClick={async () => {
                                            await saveCurrentCustomPreset()
                                        }}
                                    >
                                        {Locale('Save')}
                                    </button>
                                ...
                                >
                                    <button
                                        className="btnSquare"
                                        title={Locale(
                                            'Move selected custom preset up'
                                        )}
                                        disabled={
                                            selected_is_custom &&
                                            selected_custom_preset_index > 0
                                                ? void 0
                                                : true
                                        }
                                        onClick={async () => {
                                            await moveSelectedCustomPreset(-1)
                                        }}
                                    >
                                        {Locale('Up')}
                                    </button>
                                    <button
                                        className="btnSquare"
                                        title={Locale(
                                            'Move selected custom preset down'
                                        )}
                                        disabled={
                                            selected_is_custom &&
                                            selected_custom_preset_index <
                                                custom_preset_names.length - 1
                                                ? void 0
                                                : true
                                        }
                                        onClick={async () => {
                                    ...
                                    <button
                                        className="btnSquare"
                                        title={Locale(
                                            'Rename selected custom preset to the name in the text field'
                                        )}
                                        disabled={
                                            selected_is_custom &&
                                            preset_store.data.preset_name?.trim()
                                                ? void 0
                                                : true
                                        }
                                        onClick={async () => {
                                            await renameSelectedCustomPreset()
                                        }}
                                    >
                                        {Locale('Rename')}
                                    </button>
                                    <button
                                        ...
                                        disabled={
                                            selected_is_custom ? void 0 : true
                                        }
                                        onClick={async () => {
                                            await deleteSelectedCustomPreset()
                                        }}
                                    >
                                        {Locale('Delete')}
                                    </button>
                        ...
                        >
                            ...
                            <button
                                ...
                                id="linkWidthHeight"
                                title={Locale(
                                    'maintain the ratio between width and height slider'
                                )}
                                onClick={(evt: any) => {
                        ...
                        <div style={{ display: 'flex' }}>
                            <SpSlider
                                title={Locale(
                                    'larger value will put more emphasis on the prompt'
                                )}
                                show-value="true"
                                id="slCfgScale"
                                min="1"
                        ...
                        <SpSlider
                            title={Locale(
                                'Pix2Pix CFG Scale (larger value will put more emphasis on the image)'
                            )}
                            show-value="false"
                            id="slImageCfgScale"
                            min="0"
                        ...
                        >
                            <div style={{ display: 'flex' }}>
                                <div>
                                    ...
                                    <SpMenu
                                        title={Locale('select an upscaler model')}
                                        items={
                    ...
                    <div>
                        ...
                        >
                            {helper_store.data.b_show_sampler
                                ? Locale('Hide Samplers')
                                : Locale('Show Samplers')}{' '}
